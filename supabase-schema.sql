...
-- If upgrading an existing DB that had price/priority, run these migrations:
-- alter table wishlist_items drop column if exists price;
-- alter table wishlist_items drop column if exists priority;
-- alter table wishlist_items add column if not exists blurb text not null default '';
